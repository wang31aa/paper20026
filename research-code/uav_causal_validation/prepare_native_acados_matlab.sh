...
BLASFEO_BUILD_TARGET="${BLASFEO_BUILD_TARGET:-GENERIC}"
HPIPM_BUILD_TARGET="${HPIPM_BUILD_TARGET:-$BLASFEO_BUILD_TARGET}"
cmake -S acados -B acados/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON -DBLASFEO_TARGET="$BLASFEO_BUILD_TARGET" -DHPIPM_TARGET="$HPIPM_BUILD_TARGET" -DACADOS_INSTALL_DIR="$GITHUB_WORKSPACE/acados"
