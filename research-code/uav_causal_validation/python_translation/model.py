...
def residual_vector(x: np.ndarray, u: np.ndarray | None, parameters: SwarmParameters,
                    terminal: bool = False, edge_weights: np.ndarray | None = None) -> np.ndarray:
    ...
    if edge_weights is not None:
        weights_matrix = np.asarray(edge_weights, dtype=float)
        if weights_matrix.shape != (parameters.n, parameters.n):
            raise ValueError("edge_weights must be n by n")
        if np.any(weights_matrix < 0) or np.any(weights_matrix > 1):
    ...
    for agent in range(parameters.n):
        for rank in range(parameters.max_neighbours):
            ...
            if neighbour < 0:
                ...
                weight = 0.0 if weights_matrix is not None else 1.0
            else:
                delta = position[neighbour] - position[agent]
                weight = 1.0 if weights_matrix is None else weights_matrix[neighbour, agent]
            separation.append(weight * (delta @ delta - parameters.reference_distance**2)
                              / parameters.max_neighbours)
...
def stage_cost(x: np.ndarray, u: np.ndarray, parameters: SwarmParameters,
               edge_weights: np.ndarray | None = None) -> float:
    y = residual_vector(x, u, parameters, edge_weights=edge_weights)
    return float(np.dot(residual_weights(parameters) * y, y))


def terminal_cost(x: np.ndarray, parameters: SwarmParameters,
                  edge_weights: np.ndarray | None = None) -> float:
    y = residual_vector(x, None, parameters, terminal=True, edge_weights=edge_weights)
