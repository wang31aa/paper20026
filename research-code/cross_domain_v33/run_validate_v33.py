...
C=json.loads((R/'V33_DOMAIN_TRANSITION_CONTRACT.json').read_text());M=json.loads((ROOT/'cross_domain_v30/V30_FROZEN_MODELS.json').read_text());assert hashlib.sha256((ROOT/'cross_domain_v30/V30_FROZEN_MODELS.json').read_bytes()).hexdigest()==C['unchanged_model_sha256'];O=R/'results';O.mkdir(exist_ok=True);rows=[]
for d,design in C['design'].items():
 run.C['domains']=[d];run.C['rho_grid']=design['rho_grid'];run.C['policies']=C['policies'];rows+=run.execute(design['sizes'],C['seeds'],O/f'v33_{d}_runs.csv')
def feat(x):
